...
namespace GameCore.Extensions
{
    ...
    public class OperationSum : IValue
    {
        ...
        public float Value
        {
            get
            {
                ...
                return m_Values.Aggregate(sum, (current, value) => current + value.Value);
            }
        }
    ...
    public class OperationReciprocal : IValue
    {
        ...
        public float Value
        {
            get { return m_Values[0].Value / m_Values[1].Value; }
        }

        public OperationReciprocal(IValue[] aValues)
        {
            m_Values = aValues;
        }
...
//            else
            ...
            else if (aExpression.Contains('/'))
            {
                ...
                if (!string.IsNullOrEmpty(parts[0].Trim()))
                    exp.Add(Parse(parts[0]));
                return new OperationReciprocal(new[] {Parse(parts[0]), Parse(parts[1])});
//                for (int i = 1; i < parts.Length; i++)
//                {
//                    string s = parts[i].Trim();
//                    if (!string.IsNullOrEmpty(s))
...
//                }
            ...
            float floatValue;
            if (float.TryParse(aExpression.Replace(".", ","), out floatValue))
            {
